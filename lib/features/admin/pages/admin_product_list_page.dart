...
class _AdminProductListPageState extends State<AdminProductListPage> {
  ...
  List<DummyProduct> _filteredProducts = [];

  @override
  void initState() {
    super.initState();
    _filteredProducts = List.from(DummyData.products);
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        _filteredProducts = DummyData.products
            .where((p) => p.title.toLowerCase().contains(query) || p.sku.toLowerCase().contains(query))
            .toList();
      }
    });
  }

  void _deleteProduct(DummyProduct product) {
    ConfirmationDialog.show(
      ...
      onConfirm: () {
        setState(() {
          DummyData.products.removeWhere((p) => p.id == product.id);
      ...
      appBar: CustomAppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              Navigator.pushNamed(context, AppRoutes.adminAddProduct).then((_) {
                setState(() {
                  _onSearchChanged(); // refresh on return
                });
              });
            },
          ),
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            color: (isLowStock ? theme.colorScheme.tertiary : Colors.teal).withOpacity(0.12),
